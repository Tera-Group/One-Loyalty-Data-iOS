...
@interface OneloyaltyDeviceCompanion : OneloyaltyBase
+ (instancetype)alloc __attribute__((unavailable));
+ (instancetype)allocWithZone:(struct _NSZone *)zone __attribute__((unavailable));
+ (instancetype)companion __attribute__((swift_name("init()")));
@property (class, readonly, getter=shared) OneloyaltyDeviceCompanion *shared __attribute__((swift_name("shared")));
- (OneloyaltyDevice *)fromStringJson:(NSString *)json __attribute__((swift_name("fromString(json:)")));
- (id<OneloyaltyKotlinx_serialization_coreKSerializer>)serializer __attribute__((swift_name("serializer()")));
@end
